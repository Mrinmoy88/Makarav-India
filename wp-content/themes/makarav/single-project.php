...
<!-- <section class="banner inner_banner inner_banner1">
    ...
    $categories = get_the_terms(get_the_ID(), 'project_categories'); // Replace 'project_categories' with your taxonomy

    if (!is_wp_error($categories) && !empty($categories)) {
        // Display the category name inside <h2> tag
        foreach ($categories as $category) {
            echo '<h2>' . esc_html($category->name) . '</h2>';
        }
    } else {
        echo '<h3>' . esc_html('No categories found.') . '</h3>';
    }
    ?>
                    </div>
                </div>
...
<?php endif; ?>
    ...
    <div class="category">
        ...
        <div class="faq">
            <?php
            // Fetch the categories for the 'project' post type
            ...
            if (!empty($categories) && !is_wp_error($categories)) :
                foreach ($categories as $category) :
                    ...
                    $posts_count = get_term_by('id', $category->term_id, 'project_categories')->count;
